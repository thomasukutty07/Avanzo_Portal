...
export default function Employees() {
  ...
  const loadEmployees = useCallback(async () => {
    try {
      ...
      const data = res.data
      const apiUsers = extractResults(data)
      ...
      toast.error("Process interrupted while fetching personnel directory.")
    } finally {
      setIsLoading(false)
    }
  }, [page])
  ...
  const handleExport = () => {
    ...
    const csvContent = employees.map(u => 
      `"${u.name}","${u.email}","${u.role}","${u.dept}","${u.status}"`
    ).join("\n");
    const blob = new Blob([headers + csvContent], { type: 'text/csv' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", `personnel_directory.csv`);
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {employees.filter(u => u.name.toLowerCase().includes(searchQuery.toLowerCase())).map((user) => (
                    <tr 
                      key={user.id} 
            ...
            <div className="p-8">
              <AlertDialogHeader className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <AlertDialogTitle className="text-xl font-bold text-slate-900 tracking-tight">Confirm Termination</AlertDialogTitle>
                    <p className="text-[10px] font-bold text-red-600 uppercase tracking-widest mt-0.5">Sensitive Operation</p>
                  </div>
                </div>
                <AlertDialogDescription className="text-sm font-medium text-slate-500 leading-relaxed">
                  You are about to revoke system access for <span className="text-slate-900 font-bold">{userToRevoke?.name}</span>? This action is immediate and will terminate all active sessions.
                </AlertDialogDescription>
              </AlertDialogHeader>
            </div>
