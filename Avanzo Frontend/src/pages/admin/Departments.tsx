    ...
    <OrganizationAdminChrome>
      <div className="p-6 md:p-10 space-y-8 animate-in fade-in duration-500 min-h-screen font-display bg-[#fcfcfc] text-slate-900">
        ...
        <section className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {loading ? (
            [0, 1, 2].map(i => (
              <div key={i} className="flex items-center justify-between p-8 bg-white border border-slate-50 rounded-[32px] shadow-sm">
                <div className="space-y-3">
                  <div className="h-2.5 w-28 bg-slate-100 rounded-full animate-pulse" />
                  <div className="h-8 w-16 bg-slate-100 rounded-full animate-pulse" />
                </div>
                <div className="h-16 w-16 bg-slate-100 rounded-2xl animate-pulse" />
              </div>
            ))
          ) : (
            [
        ...
        {/* Department Grid or Project Detail */}
        {!selectedDept ? (
          <section className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {loading ? (
              [0, 1, 2, 3].map(i => (
                <div key={i} className="bg-white rounded-[40px] border border-slate-50 shadow-sm overflow-hidden flex flex-col">
